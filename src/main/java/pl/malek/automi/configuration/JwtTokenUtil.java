...
public class JwtTokenUtil implements Serializable {
    ...
    public String generateToken(UserDetails userDetails) throws UserNotFoundException {
        ...
        claims.put("role", userDetails.getAuthorities().stream().collect(Collectors.toList()).get(0).getAuthority());
        claims.put("id", userRepository.findByEmail(userDetails.getUsername()).orElseThrow(
                () -> new UserNotFoundException(String.format("User with email: %s", userDetails.getUsername()))
        ).getId());
